...
source: git
fetched_at: 2026-04-29T12:02:55.407204872-03:00
rendered_js: false
optimized: true
...
Extensions register custom model providers via `pi.registerProvider()`. Enables proxies, custom endpoints, OAuth/SSO for enterprise providers, and non-standard LLM API streaming.
...
The extension factory can be `async` for dynamic model discovery. pi waits for the factory before startup, so the provider is available during interactive startup and to `pi --list-models`.
...
Add a new provider with `models` and required configuration:
...
For remote model lists, use an async factory:
...
Unregistering removes that provider's dynamic models, API key fallback, OAuth registration, and custom stream handlers. Overridden built-in models or behavior are restored.

Calls after initial extension load apply immediately—no `/reload` required.
...
> [!info] Migration
> Mistral moved from `openai-completions` to `mistral-conversations`. Use `mistral-conversations` for native Mistral models.
...
- `qwen-chat-template`: local Qwen-compatible servers that read `chat_template_kwargs.enable_thinking`
- `cacheControlFormat: "anthropic"`: OpenAI-compatible providers with Anthropic-style prompt caching via `cache_control`

### Auth Header

Set `authHeader: true` for providers expecting `Authorization: Bearer <key>`:
...
Persisted in `~/.pi/agent/auth.json`:
...
Implement `streamSimple` for non-standard APIs. Study existing providers:
...
Test your provider against the same suites used by built-in providers. Copy and adapt from [packages/ai/test/](https://github.com/badlogic/pi-mono/tree/main/packages/ai/test):
...
| Option | Description |
|--------|-------------|
| `thinkingFormat: "deepseek"` | Sends `thinking: { type: "enabled" | "disabled" }` and `reasoning_effort` |
| `thinkingFormat: "qwen"` | DashScope-style top-level `enable_thinking` |
| `thinkingFormat: "qwen-chat-template"` | Local Qwen servers reading `chat_template_kwargs.enable_thinking` |
| `cacheControlFormat: "anthropic"` | Anthropic-style `cache_control` markers on system prompt, last tool definition, last user/assistant text |

## Example Extensions

See complete provider examples in the codebase:
- [`examples/extensions/custom-provider-anthropic/`](https://github.com/badlogic/pi-mono/tree/main/examples/extensions/custom-provider-anthropic)
- [`examples/extensions/custom-provider-gitlab-duo/`](https://github.com/badlogic/pi-mono/tree/main/examples/extensions/custom-provider-gitlab-duo)
- [`examples/extensions/custom-provider-qwen-cli/`](https://github.com/badlogic/pi-mono/tree/main/examples/extensions/custom-provider-qwen-cli)