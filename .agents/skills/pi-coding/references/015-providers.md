...
source: git
fetched_at: 2026-04-29T12:03:00.497834635-03:00
rendered_js: false
word_count: 591
summary: This document outlines how to configure and authenticate various AI providers in Pi, covering subscription management, API key storage, cloud service integration, and credential resolution order.
tags:
    - authentication
    - api-keys
    - oauth
...
Requires ChatGPT Plus or Pro subscription. For production, use the OpenAI Platform API.

## API Keys

### Environment Variables or Auth File

Use `/login` in interactive mode to store an API key in `auth.json`, or set via environment variable:
...
OAuth credentials are stored here after `/login` and managed automatically.
...
**Prompt caching** is enabled automatically for Claude models with recognizable model names. For application inference profiles, set `AWS_BEDROCK_FORCE_CACHE=1`:
...
**Via models.json:** Add Ollama, LM Studio, vLLM, or any provider that speaks a supported API. See [[014-models.md|Custom Models]].

**Via extensions:** For providers needing custom API implementations or OAuth flows, create an extension. See [[002-custom-provider.md|Custom Providers]] and the [custom-provider-gitlab-duo example](https://github.com/badlogic/pi-mono/tree/main/examples/extensions/custom-provider-gitlab-duo/).
