...
source: git
fetched_at: 2026-04-29T12:02:57.296773648-03:00
rendered_js: false
word_count: 1142
summary: This document provides a comprehensive list of default keyboard shortcuts for the pi agent and explains how to customize them via a configuration file.
tags:
    - keyboard-shortcuts
    - configuration
    - keybindings
...
All keyboard shortcuts can be customized via `~/.pi/agent/keybindings.json`. After editing, run `/reload` to apply changes without restarting.

The config file uses namespaced keybinding ids that pi uses internally and that extension authors use in `keyHint()` and injected `keybindings` managers. Older configs using pre-namespaced ids (`cursorUp`, `expandTools`) are migrated automatically on startup.
