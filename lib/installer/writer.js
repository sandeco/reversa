...
import { buildManifest, saveManifest as persistManifest } from './manifest.js';
...
export class Writer {
  constructor(projectRoot) {
    ...
    this.removedFiles = [];   // managed files removed during reconciliation
  }
  ...
  async installSkill(agentId, skillsDir) {
    ...
    this.registerPath(dest);           // directory → uninstall tracking
  ...
  updateGitignore(outputFolder) {
    ...
    if (existsSync(gitignorePath)) {
      ...
      this.registerPath(gitignorePath);
    }
  }
  ...
  saveCreatedFiles() {
    ...
    const removed = new Set(this.removedFiles.map((path) => path.replace(/\\/g, '/')));
    state.created_files = [...new Set([...(state.created_files ?? []), ...this.createdFiles])]
      .filter((path) => !removed.has(path.replace(/\\/g, '/')));
    writeFileSync(statePath, JSON.stringify(state, null, 2), 'utf8');
    this.registerPath(statePath);
  }

  saveManifest(existingManifest = {}) {
    const reconciled = { ...existingManifest };
    for (const removedPath of this.removedFiles) delete reconciled[removedPath];
    Object.assign(reconciled, buildManifest(this.projectRoot, this.manifestPaths));
    persistManifest(this.projectRoot, reconciled);
  }
}
