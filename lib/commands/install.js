...
export default async function install(args) {
  ...
  // Validate every routed engine before the installer mutates the project.
  validateModelRouting(projectRoot, answers.engines, answers.agents);
  ...
  try {
    ...
    const routingResults = installModelRoutingProfiles(
      writer,
      answers.engines,
      answers.agents,
      existingManifest,
    );
    for (const [engineId, result] of Object.entries(routingResults)) {
      const preserved = result.profiles.filter(p => p.status === 'modified' || p.status === 'user-owned');
      for (const profile of preserved) {
        console.warn(`  ${engineId} profile preserved (${profile.status}): ${profile.relativePath}`);
      }
    }
  ...
  if (selectedEngines.length > 0) {
    ...
    const namesStr = names.length > 1
      ...
      : names[0];
    const hasSlashEngine = selectedEngines.some(e => e.id !== 'codex' && e.id !== 'hermes');
